...
class CollaborativeFiltering:
    ...
    def save_recommendations(self, user_id, recommendations):
        try:
            conn = mysql.connector.connect(**self.db_config)
            cursor = conn.cursor()
            cursor.execute("DELETE FROM collab_recommendations WHERE Member_ID = %s", (user_id,))
            for isbn, score in recommendations:
                cursor.execute("""
                    INSERT INTO collab_recommendations (Member_ID, ISBN, Score)
                    VALUES (%s, %s, %s)
            ...
            logging.error(f"Error saving collaborative recommendations: {str(e)}")
    ...
    def get_recommendations(self, user_id, n_recommendations=12):
        try:
            ...
            recommendations = [(item, score) for item, score in sorted(cf_scores.items(), key=lambda x: x[1], reverse=True)]
