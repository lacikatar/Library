...
try {
    ...
    $totalBooks = $pdo->query("SELECT COUNT(*) as count FROM book")->fetch()['count'];
    file_put_contents($logFile, "Total books in database: $totalBooks\n", FILE_APPEND);
    ...
    do {
        ...
        foreach ($books as $book1) {
            ...
            $similarBooks = $pdo->prepare($similarQuery);
            $similarBooks->execute([
                $book1['ISBN'], // For tag count
                $book1['ISBN'], // For category count
                $book1['ISBN'], // For author count
                $book1['ISBN'], // For WHERE clause
                $book1['ISBN']  // For HAVING clause
            ]);
            $similarBooks = $similarBooks->fetchAll(PDO::FETCH_ASSOC);
